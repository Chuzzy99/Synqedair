  ...
  useEffect(() => {
    ...
    fetch("https://ipapi.co/json/")
      ...
      .then((d) => {
        ...
        if (loc) {
          setUserLocation(loc);
          localStorage.setItem("synqed_last_location", loc);
        }
      })
      .catch(() => {});
  }, []);
        ...
        <section className="mx-auto max-w-6xl px-6 md:px-10 pt-12 pb-16 md:pt-20 md:pb-24 grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="max-w-xl">
            ...
            <div className="hidden md:block">
              ...
              <motion.h1 variants={fadeUp} className="font-display text-4xl md:text-5xl lg:text-7xl font-semibold leading-[1.06] tracking-tight mb-4 md:mb-6">
                Where to next?
              </motion.h1>
            ...
            <motion.div variants={fadeUp} className="mt-8 md:mt-12 bg-white rounded-3xl p-6 md:p-8 shadow-[0_20px_40px_-18px_rgba(0,0,0,0.6)]">
                  ...
                  {["Round trip", "One way", "Multi-city"].map((type) => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
                        tripType === type ? "bg-white text-indigo shadow-sm" : "text-mist hover:text-ink"
                      }`}
                    >
                      {type}
                    </button>
              ...
              >
                ...
                {tripType === "Multi-city" && (
                  <button type="button" className="text-sm font-semibold text-[#1C9BB8] hover:text-[#157a91] text-left self-start mt-2">
                    + Add another flight
                  </button>
                )}
              </form>
          ...
          {/* Right col — price transparency + quick routes */}
          <motion.div variants={staggerContainer} initial="hidden" animate="show" className="max-w-md w-full mx-auto md:ml-auto md:mr-0">
            ...
            <motion.div variants={fadeUp} className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => router.push("/search")} className="text-sm text-ice font-semibold hover:underline">
                see all →
              </button>
            </motion.div>
                ...
                {CORRIDORS.map((c) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {c.tag && (
                        <span className="text-[10px] font-bold uppercase tracking-wider bg-indigo/5 text-indigo px-2 py-0.5 rounded-full">
                          {c.tag}
                        </span>
                      )}
                    </div>
                    <div className="font-display font-semibold text-base text-ink mb-1">
                      {c.from} → {c.to}
                    </div>
                ...
                {[
                  {
                    ...
                    body: "What you see is what you pay. Base fare, taxes, and baggage shown explicitly on every result — not added at checkout.",
                    tag: null,
                  },
                  {
                    ...
                    body: "Reach a real Synqed Air agent in under 2 minutes on WhatsApp. Delays, changes, and refunds handled — no bot loops, ever.",
                    tag: null,
                  },
                  {
                    ...
                    body: "Visa requirements, weather, automatic check-in, and real-time gate alerts — delivered to your phone precisely when you need them.",
                    tag: "In the app",
                  },
                ].map((p) => (
                  <motion.div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {p.tag && (
                        <span className="text-[9px] font-bold uppercase tracking-wider bg-indigo text-white px-2 py-0.5 rounded-full">
                          {p.tag}
                        </span>
                      )}
                    </div>
