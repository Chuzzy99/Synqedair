...
function mapOffer(offer: any) {
  ...
  // Note: Booking fee calculated dynamically client-side based on user location
  // Server returns base flight price without booking fee
  const fees = {
    fare:       baseAmount,
    taxes:      taxAmount,
    bags:       0,       // Duffel baggages are separate — keeping $0 for now
    bookingFee: 0,        // Will be calculated client-side
    total:      totalAmount, // Base total without booking fee
    currency,
  };
