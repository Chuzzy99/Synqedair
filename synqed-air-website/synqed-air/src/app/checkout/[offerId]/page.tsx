...
export default function CheckoutPage() {
  ...
  // Calculate prices (always in USD for display)
  ...
  const ancillariesPrice = ancillariesPayload?.services
    ...
    : 0;
  const bookingFeeUSD = 20; // Fixed $20 booking fee
  const totalPrice = basePrice + ancillariesPrice + bookingFeeUSD;
  
  // Convert to user's local currency for payment
  const convertToPaymentCurrency = (amount: number, sourceCurrency: string) => {
    if (sourceCurrency === userCurrency || Object.keys(exchangeRates).length === 0) {
      return amount;
    }
    
    const fromRate = sourceCurrency === "USD" ? 1 : exchangeRates[sourceCurrency] || 1;
    const toRate = userCurrency === "USD" ? 1 : exchangeRates[userCurrency] || 1;
    
    const amountInUSD = amount / fromRate;
    return amountInUSD * toRate;
  };
  
  // Convert to Paystack-supported currency for payment
  const paymentCurrency = getPaystackCurrency(userCurrency);
  const paymentRate = paymentCurrency === "USD" ? 1 : exchangeRates[paymentCurrency] || 1;
  const totalPriceInPaymentCurrency = totalPrice * paymentRate; // Assuming USD base
  ...
  const handlePaymentInit = async () => {
    try {
      ...
      // Call backend to initialize payment with converted amount in user's currency
      const res = await fetch(`${BACKEND_URL}/api/bookings`, {
        ...
        body: JSON.stringify({
          ...
          amount: totalPriceInPaymentCurrency,
          currency: paymentCurrency,
          callbackUrl,
          bookingFee: 20, // Send $20 booking fee for record
        }),
      });
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-line sticky top-6">
            ...
            {ancillariesPrice > 0 && (
              <div className="flex justify-between items-center py-3 border-b border-line">
                ...
                <span className="font-semibold">${ancillariesPrice.toFixed(2)}</span>
              </div>
            )}

            <div className="flex justify-between items-center py-3 border-b border-line">
              <span className="text-mist">Booking Fee</span>
              <span className="font-semibold">${bookingFeeUSD.toFixed(2)}</span>
            </div>

            <div className="flex justify-between items-center py-5 text-xl font-bold">
              <span>Total</span>
              <span className="text-indigo">${totalPrice.toFixed(2)}</span>
            </div>

            <div className="flex justify-between items-center py-3 border-t border-line bg-indigo/5 rounded-xl px-4 my-4">
              <span className="text-sm font-semibold text-indigo">Payable in {paymentCurrency}</span>
              <span className="font-bold text-lg text-indigo">{formatCurrency(totalPriceInPaymentCurrency, paymentCurrency)}</span>
            </div>

            {userCurrency !== "USD" && (
              <p className="text-xs text-mist mb-4 text-center">
                Prices shown in USD · Payment converted to {paymentCurrency}
              </p>
            )}

            {!isPaystackSupported(userCurrency) && (
              <p className="text-xs text-amber-600 mb-4 text-center bg-amber-50 p-2 rounded-xl">
                Payment will be processed in {paymentCurrency} (Paystack doesn't support {userCurrency})
              </p>
            )}
